    ...
    async.eachSeries(url_list, function (arr_url, callback) {
        ...
        request(arr_url, function (err, data) {
            ...
            $('.tptx_ytgt .tptx_ytgt_4').each(function () {
                ...
                var item = {
                    ...
                    txtCruise: arr1[1], //游轮
                    txtLine: analyLine($me.find('.tptx_jcyj_2ab_1 ul li:first-child').text()),// 航线
                    txtStartDate: analyStart($me.find('.tptx_jcyj_2ab_1 li').eq(1).text(),
                        $me.find('.tptx_jcyj_2ab_1 span').text()),
                    numDay: Number(arr2[1]), // 天数
                    numNight: Number(arr2[0]), //夜数
                    numPrice: analyPrice($me.find('.tptx_jcyj_2ac .tptx_jcyj_2ac_1').text()),// 价格
        ...
        function (err) {
            ...
            async.eachSeries(list, function (record, callback) {
                ...
                console.log(str)
                // 拼接sql
                var sql = 'INSERT INTO tb_crawl_boat_ticket (company,cruise,line,state_date,boat_day,night,price,url)values(' + str + ')';
                dbUtil.EXECUTE(sql).then(res => {
                    console.log('res==>', res)
                    // 受影响的行数
